...
let load_missing () =
  let s, w = App.load_manifest (mem_fs (Hashtbl.create 1)) "pkgs.txt" in
...
let load_parses () =
  ...
  Hashtbl.add files "pkgs.txt" "# winget: C:\\w\\winget.exe\n# tools\nwinget:Git.Git\n";
  let s, w = App.load_manifest (mem_fs files) "pkgs.txt" in
...
let append_groups () =
  ...
  Alcotest.(check (result unit string))
    "ok"
    (Ok ())
    (App.append_selected (mem_fs files) "pkgs.txt" items);
  Alcotest.(check string)
    "grouped under header"
    "\n# Newly detected\nwinget:Brave.Brave\nwinget:Git.Git\n"
    (Hashtbl.find files "pkgs.txt")
;;
...
let append_empty () =
  ...
  Alcotest.(check (result unit string))
    "ok"
    (Ok ())
    (App.append_selected (mem_fs files) "pkgs.txt" []);
  Alcotest.(check bool) "no write" false (Hashtbl.mem files "pkgs.txt")
;;

let add () =
  let files = Hashtbl.create 1 in
  Hashtbl.add files "pkgs.txt" "# tools\nwinget:Git.Git\n";
  ...
  Alcotest.(check (list string))
    ...
    ; "  appended 1 app(s) to pkgs.txt"
    ]
    msgs;
  Alcotest.(check bool)
    "brave persisted"
    true
    (contains "winget:Brave.Brave" (Hashtbl.find files "pkgs.txt"))
;;
...
let export () =
  let files = Hashtbl.create 1 in
  let msgs = App.run_export (env files) "out.txt" in
  Alcotest.(check (list string))
    "messages"
    [ "wrote out.txt (2 apps)"; "wrote out.json (2 winget apps, winget import -i ready)" ]
    msgs;
  Alcotest.(check string)
    "content"
    "# Generated by devkit export\n\n# winget\nwinget:Brave.Brave\nwinget:Git.Git\n"
    (Hashtbl.find files "out.txt");
...
let export_no_winget () =
  ...
  in
  let msgs = App.run_export e "out.txt" in
  Alcotest.(check bool)
    "skip message"
    true
...
let new_items () =
  ...
  in
  let secs =
    [ { Pkgfile.name = "Newly detected"; items = [ mk Pkgfile.New ] }
    ; { Pkgfile.name = "Pending updates"; items = [ mk Pkgfile.New ] }
    ]
  in
